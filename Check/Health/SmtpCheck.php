 ...
 *
 * W Magento to szczególnie delikatne miejsce: wysyłkę bardzo często przejmuje
 * moduł innego producenta, który trzyma własną konfigurację. Wtedy mówimy
 * wprost, że nie mamy czego sprawdzić, zamiast zapalać zielone światło nad
 * ustawieniami, z których sklep i tak nie korzysta.
 */
class SmtpCheck implements HealthCheckInterface
{
    ...
    {
        $group = $this->scopeConfig->getValue('system/smtp');
        $config = MailConfig::parse(\is_array($group) ? $group : []);

        if (MailConfig::KIND_DISABLED === $config['kind']) {
            return CheckResult::warn('smtp', 'Wysyłka e-mail', 'Wysyłka poczty jest wyłączona w konfiguracji sklepu (Disable Email Communications). Sklep nie wyśle potwierdzeń zamówień.');
        }
        if (MailConfig::KIND_LOCAL === $config['kind']) {
            return CheckResult::warn('smtp', 'Wysyłka e-mail', sprintf(
                'Wysyłka lokalna (%s), bez serwera SMTP w konfiguracji sklepu. Nie da się jej sprawdzić z zewnątrz, a wiadomości często trafiają do niechcianych. Jeżeli pocztę wysyła moduł innego producenta, jego ustawień stąd nie widzimy.',
                $config['transport']
            ));
        }
        ...
        $result = $this->probe((string) $config['host'], (int) $config['port'], (string) $config['secure']);
